...
function initialPop(sudoku, maxPopCount = 1) {
    const population = [];
...
function calculateFitnessSum(population) {
    let sum = 0;
    population.forEach(sol => sum += sol.fitness);
    return sum;
}
function getRouletteWheel(population, k) {
...
function selectParentsFrom(population, selectionCount, k) {
...
function crossover(parents, pc, k) {
...
function mutate(population, origin, pm) {
    return population.map(sol => {
...
function print(bestSol, bestFromPop) {
    console.clear();
...
function getBestSolutions(population, bestSol) {
...
// while (!resolved) {
//   let population: SolutionData[] = initialPop(sudoku, maxPop);
//   let bestSol = getBestSolution(population);
//   maxPop = 21;
//   while (maxPop < 28 && !resolved) {
//     maxPop += 0.01;
//     let bestFromPop = getBestSolution(population);
//     bestSol = bestFromPop.fitness > bestSol.fitness ? bestFromPop : bestSol;
//     resolved = bestSol.fitness === RESOLVED_SCORE;
//     //population = population.sort(descendingSort);
//     selectedParents = selectParentsFrom(population, maxPop * 0.8, 12);
//     population = crossover([bestSol, bestFromPop, ...selectedParents], 0.8, 12);
//     mr += 0.0001;
//     population = mutate([...population, ...initialPop(sudoku, maxPop / 5)], sudoku, mr);
//     // resolved = true;
//     console.clear();
//     console.table(bestSol.sol);
//     console.log(bestSol.fitness);
...
let bestFromPop;
while (!resolved) {
    let population = initialPop(sudoku, popCount).sort(descendingSort);
    let bestSol = getBestSolution(population);
    popCount = POP_COUNT;
